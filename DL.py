...
# ⚠️ Tính metrics cho CẢ 2 CLASSES
# Class 0 = Attack, Class 1 = Benign (theo LabelEncoder alphabet order)

# Metrics cho Attack class (pos_label=0)
precision_attack = precision_score(y_test, y_pred, average='binary', pos_label=0)
recall_attack = recall_score(y_test, y_pred, average='binary', pos_label=0)
f1_attack = f1_score(y_test, y_pred, average='binary', pos_label=0)

# Metrics cho Benign class (pos_label=1)
precision_benign = precision_score(y_test, y_pred, average='binary', pos_label=1)
recall_benign = recall_score(y_test, y_pred, average='binary', pos_label=1)
f1_benign = f1_score(y_test, y_pred, average='binary', pos_label=1)
...
print("\n" + "-" * 100)
print(f"📈 METRICS CHO CLASS {le.classes_[0].upper()} (Class 0 - Majority Class)")
print("-" * 100)
print(f"  PRECISION: {precision_attack:.4f} ({precision_attack*100:.2f}%)")
print(f"  RECALL:    {recall_attack:.4f} ({recall_attack*100:.2f}%)")
print(f"  F1-SCORE:  {f1_attack:.4f} ({f1_attack*100:.2f}%)")

print("\n" + "-" * 100)
print(f"📉 METRICS CHO CLASS {le.classes_[1].upper()} (Class 1 - Minority Class)")
print("-" * 100)
print(f"  PRECISION: {precision_benign:.4f} ({precision_benign*100:.2f}%)")
print(f"  RECALL:    {recall_benign:.4f} ({recall_benign*100:.2f}%)")
...
with open(os.path.join(BACKUP_FOLDER, 'results_summary.txt'), 'w', encoding='utf-8') as f:
    ...
    f.write(f"Class {le.classes_[0]} (Attack - Majority Class):\n")
    f.write(f"  Precision: {precision_attack:.4f} ({precision_attack*100:.2f}%)\n")
    f.write(f"  Recall:    {recall_attack:.4f} ({recall_attack*100:.2f}%)\n")
    f.write(f"  F1-Score:  {f1_attack:.4f} ({f1_attack*100:.2f}%)\n\n")
    f.write(f"Class {le.classes_[1]} (Benign - Minority Class):\n")
    f.write(f"  Precision: {precision_benign:.4f} ({precision_benign*100:.2f}%)\n")
    f.write(f"  Recall:    {recall_benign:.4f} ({recall_benign*100:.2f}%)\n")
    f.write(f"  F1-Score:  {f1_benign:.4f} ({f1_benign*100:.2f}%)\n\n")
...
print(f"\n  Class {le.classes_[0]} (Attack):")
print(f"    ✓ Precision: {precision_attack:.4f} ({precision_attack*100:.2f}%)")
print(f"    ✓ Recall:    {recall_attack:.4f} ({recall_attack*100:.2f}%)")
print(f"    ✓ F1-Score:  {f1_attack:.4f} ({f1_attack*100:.2f}%)")
print(f"\n  Class {le.classes_[1]} (Benign):")
print(f"    ✓ Precision: {precision_benign:.4f} ({precision_benign*100:.2f}%)")
print(f"    ✓ Recall:    {recall_benign:.4f} ({recall_benign*100:.2f}%)")
print(f"    ✓ F1-Score:  {f1_benign:.4f} ({f1_benign*100:.2f}%)")
